            ...
            <div class="px-6 py-4 border-b border-gray-200 dark:border-neutral-700 flex items-center">
                ...
                <div class="ms-3">
                    <h2 class="text-xl font-semibold text-gray-800 dark:text-neutral-200">
                        {{ $page['title'] }}: <span class="uppercase">{{ $profile->type }}</span>
                    </h2>
                </div>
            </div>

            <form navigate-form action="{{ route('superadmin.profiles.doUpdate', $profile->id) }}" method="POST" class="p-6">
                @csrf
                <div class="space-y-4">
                    <!-- Form Group -->
                    <div>
                        <label class="block text-sm font-medium mb-2 dark:text-white text-gray-400">Tipe Profil</label>
                        <div class="py-3 px-4 block w-full bg-gray-50 border-gray-200 rounded-lg text-sm dark:bg-neutral-900/50 dark:border-neutral-700 dark:text-neutral-500 font-bold uppercase tracking-widest">
                            {{ $profile->type }}
                        </div>
                    </div>
                    <!-- End Form Group -->
                    ...
                    <div class="prose max-w-none dark:prose-invert">
                        ...
                        <x-trix-input id="content" name="content" :value="old('content', $profile->content)" />
                        @error('content')
