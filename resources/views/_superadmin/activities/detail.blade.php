        ...
        <div
            ...
            <div class="p-6">
                ...
                <div class="mb-6">
                    ...
                    @endphp
                    <span class="inline-flex items-center gap-x-1.5 py-1.5 px-3 rounded-full text-xs font-bold uppercase tracking-wider {{ $currentColor }}">
                        {{ $activity->status }}
                    </span>
                    @if ($activity->trashed())
                        <span class="ms-2 inline-flex items-center gap-x-1.5 py-1.5 px-3 rounded-full text-xs font-bold bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-500 uppercase tracking-wider">
                            Terhapus (Sampah)
                        </span>
                    @endif
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    ...
                    <div class="lg:col-span-2 space-y-8">
                        ...
                        @if ($activity->poster)
                            <div class="rounded-2xl overflow-hidden shadow-xl border border-gray-100 dark:border-neutral-700 bg-gray-50 dark:bg-neutral-900">
                    ...
                    <!-- Sidebar Info -->
                    <div class="space-y-6">
                        <div class="bg-gray-50 dark:bg-neutral-700/30 rounded-2xl p-6 space-y-5 border border-gray-100 dark:border-neutral-700/50 shadow-sm transition-all hover:shadow-md">
                            <h3 class="text-sm font-bold uppercase tracking-widest text-gray-400 dark:text-neutral-500 border-b border-gray-200 dark:border-neutral-700 pb-3">
                                Detail Pelaksanaan
                            </h3>

                            <div class="flex items-start gap-3">
                                <div class="p-2 rounded-lg bg-brand/10 text-brand">
                                    <svg class="size-5" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                                </div>
                                <div>
                            ...
                            <div class="flex items-start gap-3 pt-2 border-t border-gray-200 dark:border-neutral-700">
                                <div class="size-9 rounded-full bg-brand/10 flex items-center justify-center text-brand font-bold text-xs">
                                    {{ substr($activity->creator->name ?? 'A', 0, 1) }}
                                </div>
                                <div>
                                    <h4 class="text-xs font-bold text-gray-400 dark:text-neutral-500 uppercase">Penyelenggara</h4>
                                    <p class="text-sm font-bold text-gray-800 dark:text-neutral-200">
            ...
            <div class="relative flex flex-col bg-white border border-gray-200 shadow-sm rounded-2xl dark:bg-neutral-800 dark:border-neutral-700 overflow-hidden">
                <div class="p-4 sm:p-10 text-center">
                    ...
                    <p class="text-gray-500 dark:text-neutral-500 px-6">
                        Apakah Anda yakin ingin menghapus kegiatan <span class="font-bold text-gray-800 dark:text-neutral-200">{{ $activity->title }}</span>?
                        <br>
                    ...
                    <div class="mt-8 flex justify-center gap-x-3">
                        ...
                        <form action="{{ $activity->trashed() ? route('superadmin.activities.forceDelete', $activity->id) : route('superadmin.activities.delete', $activity->id) }}" method="POST" class="inline" navigate-form>
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="py-2.5 px-6 inline-flex items-center gap-x-2 text-sm font-semibold rounded-xl border border-transparent {{ $activity->trashed() ? 'bg-rose-600 hover:bg-rose-700' : 'bg-red-600 hover:bg-red-700' }} text-white cursor-pointer shadow-md shadow-red-500/20 transition-all active:scale-95">
                                Ya, Hapus Data
                            </button>
                        </form>
                    </div>
