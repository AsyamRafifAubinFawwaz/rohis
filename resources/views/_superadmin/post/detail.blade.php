            ...
            <div class="px-6 py-4 border-b border-gray-200 dark:border-neutral-700 flex items-center justify-between">
                ...
                <div class="flex items-center gap-x-2">
                    @if ($post->status != 'published')
                        <form action="{{ route('superadmin.posts.approve', $post->id) }}" method="POST" navigate-form>
                            @csrf
                            <button type="submit"
                                class="py-2.5 px-5 inline-flex items-center gap-x-2 text-sm font-semibold rounded-xl border border-transparent bg-emerald-100 text-emerald-800 hover:bg-emerald-200 focus:outline-none focus:bg-emerald-200 transition-all cursor-pointer shadow-sm">
                    ...
                    @if ($post->status == 'pending')
                        <form action="{{ route('superadmin.posts.reject', $post->id) }}" method="POST" navigate-form>
                            @csrf
                            <button type="submit"
                                class="py-2.5 px-5 inline-flex items-center gap-x-2 text-sm font-semibold rounded-xl border border-transparent bg-red-100 text-red-800 hover:bg-red-200 focus:outline-none focus:bg-red-200 transition-all cursor-pointer shadow-sm">
                    ...
                    @endif
                    @if ($post->status == 'published')
                        <form action="{{ route('superadmin.posts.draft', $post->id) }}" method="POST" navigate-form>
                            @csrf
                            <button type="submit"
                                class="py-2.5 px-5 inline-flex items-center gap-x-2 text-sm font-semibold rounded-xl border border-transparent bg-amber-100 text-amber-800 hover:bg-amber-200 focus:outline-none focus:bg-amber-200 transition-all cursor-pointer shadow-sm">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="space-y-6">
                        <div
                            ...
                            <div>
                                <h4 class="text-xs font-medium text-gray-500 dark:text-neutral-500">Penulis</h4>
                        ...
                        <div class="flex flex-col gap-2">
                            @if ($post->trashed())
                                <form action="{{ route('superadmin.posts.restore', $post->id) }}" method="POST"
                                    navigate-form>
                                    @csrf
                                    <button type="submit"
                                        class="w-full py-3 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-xl border border-transparent bg-indigo-100 text-indigo-800 hover:bg-indigo-200 focus:outline-none shadow-sm transition-all active:scale-95 cursor-pointer">
                                        @include('_admin._layout.icons.reset')
                                        Pulihkan Postingan
                                    </button>
                                </form>
                                <button type="button"
                                    class="w-full py-3 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-xl border border-transparent bg-rose-100 text-rose-800 hover:bg-rose-200 focus:outline-none transition-all cursor-pointer shadow-sm active:scale-95"
            ...
            <div
                ...
                <div class="p-4 sm:p-10 text-center">
                    ...
                    <h3 class="mb-2 text-xl font-bold text-gray-800 dark:text-neutral-200">
                        {{ $post->trashed() ? 'Hapus Permanen' : 'Hapus Postingan' }}
                    </h3>
                    <p class="text-gray-500 dark:text-neutral-500">
                        Apakah Anda yakin ingin menghapus postingan ini?
                        <br>
                        {{ $post->trashed() ? 'Tindakan ini tidak dapat dibatalkan.' : 'Postingan akan dipindahkan ke sampah.' }}
                    </p>
                    <div class="mt-6 flex justify-center gap-x-4">
                        ...
                        <form
                            action="{{ $post->trashed() ? route('superadmin.posts.forceDelete', $post->id) : route('superadmin.posts.delete', $post->id) }}"
                            method="POST" class="inline" navigate-form>
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                class="py-2.5 px-4 inline-flex items-center gap-x-2 text-sm font-semibold rounded-xl border border-transparent {{ $post->trashed() ? 'bg-rose-600 hover:bg-rose-700' : 'bg-red-600 hover:bg-red-700' }} text-white cursor-pointer shadow-md shadow-rose-500/20 transition-all active:scale-95">
                                {{ $post->trashed() ? 'Ya, Hapus Permanen' : 'Ya, Pindahkan ke Sampah' }}
                            </button>
                        </form>
                    </div>
                </div>
