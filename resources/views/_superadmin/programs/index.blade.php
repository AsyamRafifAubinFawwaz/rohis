...
@section('content')
            ...
            <table class="w-full divide-y divide-gray-200 dark:divide-neutral-700">
                <thead class="bg-gray-50 dark:bg-neutral-900/50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-start">
                            <span class="text-xs font-bold uppercase text-gray-500 dark:text-neutral-400">Status</span>
                        </th>
                        <th scope="col" class="px-6 py-3 text-start">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-neutral-700/50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-x-4">
                                    <div class="size-10 rounded-xl overflow-hidden bg-gray-100 dark:bg-neutral-700">
                                        @if($program->image)
                                            <img src="{{ asset('storage/' . $program->image) }}" class="size-full object-cover" alt="{{ $program->name }}">
                                        @else
                                            <div class="size-full flex items-center justify-center text-gray-400">
                                                <svg class="size-5" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/></svg>
                                            </div>
                                        @endif
                                    </div>
                                    <div>
                            ...
                            <td class="px-6 py-4 text-start">
                                @if ($program->status == 'active')
                                    <span class="inline-flex items-center gap-x-1.5 py-1 px-3 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-800/30 dark:text-blue-400 uppercase tracking-wider">
                                        <span class="size-1.5 rounded-full bg-blue-600"></span>
                                        Berjalan
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-x-1.5 py-1 px-3 rounded-full text-xs font-medium bg-gray-100 text-gray-600 dark:bg-neutral-700 dark:text-neutral-400 uppercase tracking-wider">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-x-2">
                                    <div class="size-7 rounded-full bg-brand/10 text-brand flex items-center justify-center text-[10px] font-bold uppercase">
                                ...
                                <div class="flex justify-end gap-1.5">
                                    ...
                                    @if ($program->trashed())
                                        ...
                                        <button type="button" class="p-2 rounded-lg bg-rose-50 text-rose-600 hover:bg-rose-100 dark:bg-rose-900/20 dark:text-rose-500 transition-all active:scale-90" 
                                            data-hs-overlay="#delete-modal" onclick="setDeleteData('{{ $program->id }}', '{{ addslashes($program->name) }}', true)" title="Hapus Permanen">
                                    ...
                                    @else
                                        ...
                                        <button type="button" class="p-2 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 dark:bg-red-900/20 dark:text-red-500 transition-all active:scale-90" 
                                            data-hs-overlay="#delete-modal" onclick="setDeleteData('{{ $program->id }}', '{{ addslashes($program->name) }}', false)" title="Hapus (Pindah ke Sampah)">
                                            @include('_admin._layout.icons.trash')
                                        </button>
                                    @endif
                                </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-20 text-center">
                                <x-admin.empty-state title="Belum Ada Program Kerja" description="Mulai susun program kerja pertama Anda untuk menggerakkan roda organisasi." />
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto">
            <div class="relative flex flex-col bg-white border border-gray-200 shadow-sm rounded-2xl dark:bg-neutral-800 dark:border-neutral-700 overflow-hidden">
                <div class="p-4 sm:p-10 text-center">
                    <span id="delete-icon-wrapper" class="mb-4 inline-flex justify-center items-center size-14 rounded-full border-4 border-red-50 bg-red-100 text-red-500 dark:bg-red-700/30 dark:border-red-600 dark:text-red-100">
                    ...
                    <p id="delete-modal-description" class="text-gray-500 dark:text-neutral-500 px-6">
                        Apakah Anda yakin ingin menghapus program <span id="delete-item-name" class="font-bold text-gray-800 dark:text-neutral-200"></span>? <span id="delete-alert-text">Data yang dihapus akan dipindahkan ke sampah.</span>
                    </p>
    ...
    <script>
        ...
        function setRestoreData(id, name) {
            ...
            document.getElementById('restore-form').action = `{{ url('superadmin/programs/restore') }}/${id}`;
        }
        ...
        function setDeleteData(id, name, isPermanent = false) {
            ...
            if (isPermanent) {
                form.action = `{{ url('superadmin/programs/force-delete') }}/${id}`;
                title.textContent = 'Hapus Permanen Program';
                alertText.textContent = 'Program akan dihapus secara permanen. Tindakan ini tidak dapat dibatalkan.';
                submitBtn.classList.replace('bg-red-600', 'bg-rose-600');
                submitBtn.textContent = 'Ya, Hapus Permanen';
            } else {
                form.action = `{{ url('superadmin/programs/delete') }}/${id}`;
                title.textContent = 'Hapus Program Pekerjaan';
                alertText.textContent = 'Program akan dipindahkan ke tempat sampah.';
